...
export interface UseConfigReturn extends ConfigData {
  /**
   * Build the inline SVG fragment for a warning icon.
   *
   * The optional `instanceKey` is used to prefix the `id` attributes
   * that live inside icons which internally reference themselves (sea
   * water height high/low — masks and <use>). When a map contains
   * several of those icons each one must get a unique prefix, otherwise
   * the browser's SVG *image decoder* refuses to load the document via
   * a Blob URL because of duplicate `id` values. Inline rendering is
   * forgiving, image-element loading is not.
   */
  warningIcon: (
    warning: WarningIconInput,
    instanceKey?: string
  ) => WarningIcon
}
...
/**
 * Sanitise an arbitrary caller-provided key into a safe SVG `id`
 * fragment: only letters, digits, `_` and `-` survive. Needed for the
 * `mask`/`<use>`-bearing icons whose id attributes are referenced from
 * `url(#…)` and `href="#…"` and therefore must be unique across the
 * entire outer SVG document.
 */
 ...
 *
 * `instanceKey` is threaded through to the few icons that internally
 * use `<mask>` / `<use>` — see {@link getHighSeaLevelIcon} and
 * {@link getLowSeaLevelIcon}. Icons that do not carry internal
 * references simply ignore the parameter.
 */
  ...
  switch (warning.type) {
    case 'thunderStorm':
      ...
      icon.geom = `
        ...
        <g stroke="rgba(255, 255, 255, 1)" stroke-width="2.5px" paint-order="stroke" fill="none" fill-rule="evenodd">
    ...
    case 'rain':
      ...
      icon.geom = `
        ...
        <g stroke="rgba(255, 255, 255, 1)" stroke-width="2.5px" stroke-linecap="round" paint-order="stroke" fill="none" fill-rule="evenodd">
            <g transform="translate(-9.5, -10)">
                <g transform="translate(10.978, 17.233)">
    ...
    case 'pedestrianSafety':
      ...
      icon.geom = `
        ...
        <g stroke="rgba(255, 255, 255, 1)" stroke-width="2.5px" paint-order="stroke" fill="none" fill-rule="evenodd">
            <g transform="translate(-857, -505)">
                <g transform="translate(847.75, 497)">
                    <polygon fill="#221F20"
    ...
    case 'grassFireWeather':
      ...
      icon.geom = `
        ...
        <g stroke="rgba(255, 255, 255, 1)" stroke-width="2.5px" paint-order="stroke" fill="none" fill-rule="evenodd">
            <g transform="translate(-1133, -504)">
                <g transform="translate(1123, 497)">
                    <polygon fill="#221F20"
    ...
    case 'seaWaterHeightHighWater':
      ...
      icon.geom = getHighSeaLevelIcon(instanceKey)
      break
    case 'seaWaterHeightShallowWater':
      icon.aspectRatio = [23, 22]
      icon.geom = getLowSeaLevelIcon(instanceKey)
      break
    case 'seaWaveHeight':
...
/**
 * High sea level warning icon. Its three wavy paths are stencilled
 * through `<mask>` + `<use>` references to `<polygon>` shapes in
 * `<defs>`, so those id attributes must be unique across the full
 * outer SVG document. `instanceKey` becomes the shared prefix.
 */
function getHighSeaLevelIcon(instanceKey: string): string {
  const p = `hsl-${safeId(instanceKey)}`
  return `
    <title>highsealevel-symbol</title>
    <defs>
        <polygon id="${p}-path-1"
                 points="21.914 4.508 21.914 0.04 0 0.04 0 4.508 21.914 4.508"></polygon>
        <polygon id="${p}-path-3"
                 points="21.914 4.483 21.914 0.015 0 0.015 0 4.483 21.914 4.483"></polygon>
        <polygon id="${p}-path-5"
                 points="10.957 4.484 21.914 4.484 21.914 0.016 0 0.016 0 4.484 10.957 4.484"></polygon>
    </defs>
    <g stroke="rgba(255, 255, 255, 1.0)" stroke-width="2.5px" paint-order="stroke" fill="none" fill-rule="evenodd">
        <g transform="translate(1.444, 0.444)">
            <g transform="translate(0, 12.803)">
                <mask id="${p}-mask-2" fill="white">
                    <use href="#${p}-path-1"></use>
                </mask>
                <path d="M16.435,4.508 C14.731,4.508 13.844,3.562 13.132,2.803 C12.495,2.123 11.992,1.586 10.957,1.586 C9.922,1.586 9.419,2.123 8.782,2.803 C8.069,3.562 7.183,4.508 5.478,4.508 C3.774,4.508 2.887,3.562 2.175,2.803 C1.538,2.123 1.035,1.586 0,1.586 L0,0.04 C1.704,0.04 2.591,0.985 3.303,1.745 C3.94,2.425 4.444,2.962 5.478,2.962 C6.513,2.962 7.016,2.425 7.654,1.745 C8.366,0.985 9.252,0.04 10.957,0.04 C12.661,0.04 13.548,0.985 14.26,1.745 C14.897,2.425 15.401,2.962 16.435,2.962 C17.47,2.962 17.974,2.425 18.611,1.745 C19.323,0.985 20.21,0.04 21.914,0.04 L21.914,1.586 C20.879,1.586 20.376,2.123 19.739,2.803 C19.026,3.562 18.14,4.508 16.435,4.508"
                      fill="#221F20" mask="url(#${p}-mask-2)"></path>
            </g>
            <g transform="translate(0, 9.144)">
                <mask id="${p}-mask-4" fill="white">
                    <use href="#${p}-path-3"></use>
                </mask>
                <path d="M16.435,4.483 C14.731,4.483 13.844,3.537 13.132,2.778 C12.495,2.098 11.992,1.561 10.957,1.561 C9.922,1.561 9.419,2.098 8.782,2.778 C8.069,3.537 7.183,4.483 5.478,4.483 C3.774,4.483 2.887,3.537 2.175,2.778 C1.538,2.098 1.035,1.561 0,1.561 L0,0.015 C1.704,0.015 2.591,0.961 3.303,1.72 C3.94,2.4 4.444,2.937 5.478,2.937 C6.513,2.937 7.016,2.4 7.654,1.72 C8.366,0.961 9.252,0.015 10.957,0.015 C12.661,0.015 13.548,0.961 14.26,1.72 C14.897,2.4 15.401,2.937 16.435,2.937 C17.47,2.937 17.974,2.4 18.611,1.72 C19.323,0.961 20.21,0.015 21.914,0.015 L21.914,1.561 C20.879,1.561 20.376,2.098 19.739,2.778 C19.026,3.538 18.14,4.483 16.435,4.483"
                      fill="#221F20" mask="url(#${p}-mask-4)"></path>
            </g>
            <g transform="translate(0, 16.359)">
                <mask id="${p}-mask-6" fill="white">
                    <use href="#${p}-path-5"></use>
                </mask>
                <path d="M16.435,4.484 C14.731,4.484 13.844,3.538 13.132,2.779 C12.495,2.099 11.992,1.562 10.957,1.562 C9.922,1.562 9.419,2.099 8.782,2.779 C8.069,3.538 7.183,4.484 5.478,4.484 C3.774,4.484 2.887,3.538 2.175,2.779 C1.538,2.099 1.035,1.562 0,1.562 L0,0.016 C1.704,0.016 2.591,0.961 3.303,1.721 C3.94,2.401 4.444,2.938 5.478,2.938 C6.513,2.938 7.016,2.401 7.654,1.721 C8.366,0.961 9.252,0.016 10.957,0.016 C12.661,0.016 13.548,0.961 14.26,1.721 C14.897,2.401 15.401,2.938 16.435,2.938 C17.47,2.938 17.974,2.401 18.611,1.721 C19.323,0.961 20.21,0.016 21.914,0.016 L21.914,1.562 C20.879,1.562 20.376,2.099 19.739,2.779 C19.026,3.538 18.14,4.484 16.435,4.484"
                      fill="#221F20" mask="url(#${p}-mask-6)"></path>
            </g>
            <polygon fill="#221F20"
                     points="14.192 4.843 14.192 8.611 18.497 8.611 18.497 4.843 20.65 4.843 16.345 0 12.039 4.843"></polygon>
        </g>
    </g>`
}

/**
 * Low sea level warning icon. Same mask/use pattern as the high-sea
 * one above; `instanceKey` is used as the shared id prefix.
 */
function getLowSeaLevelIcon(instanceKey: string): string {
  const p = `lsl-${safeId(instanceKey)}`
  return `
    <title>lowsealevel-symbol</title>
    <defs>
        <polygon id="${p}-path-1"
                 points="22.022 4.53 22.022 0.04 0 0.04 0 4.53 22.022 4.53"></polygon>
        <polygon id="${p}-path-3"
                points="11.011 4.506 22.022 4.506 22.022 0.016 0 0.016 0 4.506 11.011 4.506"></polygon>
    </defs>
    <g stroke="rgba(255, 255, 255, 1.0)" stroke-width="2.5px" paint-order="stroke" fill="none" fill-rule="evenodd">
        <g transform="translate(0.444, 13.311)">
            <mask id="${p}-mask-2" fill="white">
                <use href="#${p}-path-1"></use>
             </mask>
             <path d="M16.516,4.53 C14.803,4.53 13.913,3.58 13.197,2.816 C12.556,2.133 12.051,1.594 11.011,1.594 C9.971,1.594 9.465,2.133 8.825,2.816 C8.109,3.58 7.218,4.53 5.505,4.53 C3.792,4.53 2.902,3.58 2.186,2.816 C1.545,2.133 1.04,1.594 0,1.594 L0,0.04 C1.713,0.04 2.604,0.99 3.319,1.754 C3.96,2.437 4.466,2.976 5.505,2.976 C6.545,2.976 7.051,2.437 7.691,1.754 C8.407,0.99 9.298,0.04 11.011,0.04 C12.724,0.04 13.615,0.99 14.33,1.754 C14.971,2.437 15.477,2.976 16.516,2.976 C17.556,2.976 18.062,2.437 18.702,1.754 C19.418,0.99 20.309,0.04 22.022,0.04 L22.022,1.594 C20.982,1.594 20.476,2.133 19.836,2.816 C19.12,3.58 18.229,4.53 16.516,4.53"
                   fill="#221F20" mask="url(#${p}-mask-2)"></path>
        </g>
        <g transform="translate(0.444, 16.884)">
            <mask id="${p}-mask-4" fill="white">
                <use href="#${p}-path-3"></use>
            </mask>
            <path d="M16.516,4.506 C14.803,4.506 13.913,3.556 13.197,2.792 C12.556,2.109 12.051,1.57 11.011,1.57 C9.971,1.57 9.465,2.109 8.825,2.792 C8.109,3.556 7.218,4.506 5.505,4.506 C3.792,4.506 2.902,3.556 2.186,2.792 C1.545,2.109 1.04,1.57 0,1.57 L0,0.016 C1.713,0.016 2.604,0.966 3.319,1.73 C3.96,2.413 4.466,2.952 5.505,2.952 C6.545,2.952 7.051,2.413 7.691,1.73 C8.407,0.966 9.298,0.016 11.011,0.016 C12.724,0.016 13.615,0.966 14.33,1.73 C14.971,2.413 15.477,2.952 16.516,2.952 C17.556,2.952 18.062,2.413 18.702,1.73 C19.418,0.966 20.309,0.016 22.022,0.016 L22.022,1.57 C20.982,1.57 20.476,2.109 19.836,2.792 C19.12,3.556 18.229,4.506 16.516,4.506"
                  fill="#221F20" mask="url(#${p}-mask-4)"></path>
        </g>
        <polygon fill="#221F20"
                 points="14.706 4.23 14.706 0.444 19.033 0.444 19.033 4.23 21.196 4.23 16.87 9.097 12.543 4.23"></polygon>
    </g>`
}
...
function getWaveHeightIcon(): string {
  return `
    ...
    <g stroke="rgba(255, 255, 255, 1)" stroke-width="2.5px" paint-order="stroke" fill="none" fill-rule="evenodd">
        <g transform="translate(-9.5, -10)">
            <g transform="translate(10.978, 10.164)">
  ...
  return `
    ...
    <g stroke="rgba(255, 255, 255, 1)" stroke-width="2.5px" paint-order="stroke" fill="none" fill-rule="evenodd">
        <g transform="translate(-12, -13)">
            <rect fill-opacity="0" fill="#E8E8E8" x="0" y="0" width="38" height="38" rx="100"></rect>
            <polygon fill="#221F20"
